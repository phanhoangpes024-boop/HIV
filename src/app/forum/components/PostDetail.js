...
export default function PostDetail({ postId, user, userProfile, onBack }) {
        ...
        const fetchData = async () => {
            ...
            if (postErr) {
                ...
                const { data: fb } = await supabase
                    .from('forum_posts').select('*').eq('id', postId).single();
                setPost(fb ? { ...fb, user_profiles: null } : null);
            } else {
                setPost(postData);
            }

            // Tăng view — dùng update trực tiếp, không dùng rpc
            try {
                const currentViews = (postData || postErr)?.views_count || 0;
                await supabase
                    .from('forum_posts')
                    .update({ views_count: currentViews + 1 })
                    .eq('id', postId);
            } catch (e) { /* bỏ qua lỗi view */ }

            // Fetch comments
            const { data: commentsData, error: cmtErr } = await supabase
                .from('forum_comments')
                .select('*, user_profiles (display_name, avatar_url)')
                .eq('post_id', postId)
            ...
            if (cmtErr) {
                const { data: fbCmt } = await supabase
                    .from('forum_comments').select('*').eq('post_id', postId)
                    .order('created_at', { ascending: true });
                setComments((fbCmt || []).map(c => ({ ...c, user_profiles: null })));
            } else {
    ...
    // ===== OPTIMISTIC LIKE POST (chống spam) =====
    const togglePostLike = useCallback(async () => {
        if (!user) return alert('Vui lòng đăng nhập');
        if (likingPost.current) return; // chặn spam
    ...
    // ===== OPTIMISTIC LIKE COMMENT (chống spam) =====
    const toggleCommentLike = useCallback(async (commentId) => {
        if (!user) return alert('Vui lòng đăng nhập');
        if (likingComments.current[commentId]) return; // chặn spam
        ...
        const prevCount = prevComment?.likes_count || 0;

        // Optimistic
        setLikedComments(prev => ({ ...prev, [commentId]: !wasLiked }));
        setComments(prev => prev.map(c =>
            c.id === commentId
                ? { ...c, likes_count: wasLiked ? Math.max(0, prevCount - 1) : prevCount + 1 }
                : c
        ));
        ...
        try {
            ...
            setLikedComments(prev => ({ ...prev, [commentId]: wasLiked }));
            setComments(prev => prev.map(c =>
                c.id === commentId ? { ...c, likes_count: prevCount } : c
            ));
        } finally {
        ...
        try {
            ...
            for (const img of commentImages) {
                ...
                const { error: upErr } = await supabase.storage
                    .from('forum-images')
                    .upload(fileName, img.blob, { contentType: 'image/webp', cacheControl: '31536000' });
                if (!upErr) {
            ...
            if (error) {
                // Fallback: fetch lại comment vừa tạo
                const { data: fb } = await supabase
                    .from('forum_comments')
                    .select('*')
                    .eq('post_id', postId)
                    ...
                    .single();
                if (fb) {
                    setComments(prev => [...prev, { ...fb, user_profiles: userProfile || null }]);
                }
            } else {
    ...
    const renderComment = (comment, depth = 0) => {
        ...
        return (
            <div key={comment.id} className="fm-comment" style={{ marginLeft: Math.min(depth, maxDepth) * 24 }}>
                <div className="fm-comment-inner">
                    ...
                    <div className="fm-comment-body">
                        ...
                        <div className="fm-comment-actions">
                            <button
                                className={`fm-comment-action ${likedComments[comment.id] ? 'liked' : ''}`}
                                onClick={() => toggleCommentLike(comment.id)}
                            >
                                Thích{comment.likes_count > 0 ? ` (${comment.likes_count})` : ''}
                            </button>
                            <button className="fm-comment-action" onClick={() => handleReply(comment)}>
                                Trả lời
                            </button>
                            <span className="fm-comment-time">{timeAgo(comment.created_at)}</span>
                        </div>
                    </div>
                </div>
    ...
    if (loading) {
        return (
            <div className="fm-container">
                <div className="fm-detail-loading">
                    <div className="fm-spinner"><div className="fm-spinner-dot" /></div>
                    <p>Đang tải...</p>
                </div>
            </div>
        );
    ...
    if (!post) {
        return (
            <div className="fm-container">
                ...
                <button className="fm-back-btn" onClick={onBack}>Quay lại</button>
            </div>
        );
    }
    ...
    return (
        <div className="fm-container">
            ...
            <div className="fm-detail-card">
                ...
                {/* Actions — với animation */}
                <div className="fm-detail-actions">
                    <button
                        className={`fm-action-btn ${liked ? 'liked' : ''}`}
            ...
            <div className="fm-comments-section">
                <h3 className="fm-comments-title">
                ...
                {comments.length === 0 && (
                    <p className="fm-no-comments">Chưa có bình luận nào. Hãy là người đầu tiên!</p>
                )}
